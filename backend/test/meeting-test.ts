...
describe("User", function () {
	...
	it("joins, matches into a room, and establishes RTC connection", async function () {
		mocker1.connect();
		mocker1.waitForRoom();
		await delay(1000);
		mocker2.connect();
		...
		expect(mocker1.peerConnection.localDescription).to.not.be.null;
		expect(mocker2.peerConnection.localDescription).to.not.be.null;
		expect(mocker1.peerConnection.remoteDescription).to.not.be.null;
		expect(mocker2.peerConnection.remoteDescription).to.not.be.null;
	});
});
